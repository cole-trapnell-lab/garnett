...
test_that("classify_cells works", {
  ...
  expect_equal(sum(colData(new_cds)$cluster_ext_type == "T cells"), 193)
})
...
test_that("classify_cells extension parameters work", {
  ...
  expect_equal(sum(colData(new_cds)$cell_type == "CD8 T cells"), 61)
  expect_equal(sum(colData(new_cds)$cell_type == "T cells"), 164)
  expect_equal(sum(colData(new_cds)$cluster_ext_type == "B cells"), 207)
  expect_equal(sum(colData(new_cds)$cluster_ext_type == "CD4 T cells"), 1)
  expect_equal(sum(colData(new_cds)$cluster_ext_type == "T cells"), 400)

  expect_identical(counts(new_cds2), counts(test_cds))
  expect_identical(rowData(new_cds2), rowData(test_cds))
  expect_equal(sum(colData(new_cds2)$cell_type == "B cells"), 207)
  expect_equal(sum(colData(new_cds2)$cell_type == "CD4 T cells"), 129)
  expect_equal(sum(colData(new_cds2)$cell_type == "CD8 T cells"), 61)
  expect_equal(sum(colData(new_cds2)$cell_type == "T cells"), 164)
