...
test_that("classify_cells marker_free works", {
  ...
  expect_equal(sum(colData(new_cds)$cell_type == "B cells"), 368)
  expect_equal(sum(colData(new_cds)$cell_type == "CD4 T cells"), 218)
  expect_equal(sum(colData(new_cds)$cell_type == "CD8 T cells"), 114)
  ...
  expect_equal(sum(colData(new_cds)$cluster_ext_type == "T cells"), 398)
})
